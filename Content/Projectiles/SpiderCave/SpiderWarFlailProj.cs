...
{
	public class SpiderWarFlailProj : ModProjectile
	{	
		...
		{
            ChainTexture ??= ModContent.Request<Texture2D>("Spooky/Content/Projectiles/SpiderCave/SpiderWarFlailChain");
		...
		public override void AI() 
        {
			...
			switch (CurrentAIState) 
            {
				case AIState.Spinning: 
                {
					...
					RocketTimer++;
					if (RocketTimer % 10 == 0 && Main.myPlayer == Projectile.owner)
					{
						...
						{
							Dust newDust = Dust.NewDustPerfect(Projectile.Center, DustID.InfernoFork, new Vector2(ShootSpeed.X + Main.rand.Next(-7, 8), ShootSpeed.Y + Main.rand.Next(-7, 8)) * 0.5f, default, default, 1f);
		...
		public override bool OnTileCollide(Vector2 oldVelocity) 
        {
			...
			if (CurrentAIState == AIState.Dropping)
			{
				if (!HasSlammed)
				{
					SoundEngine.PlaySound(SoundID.DD2_MonkStaffGroundImpact with { Volume = 0.5f }, Projectile.Center);
					Screenshake.ShakeScreenWithIntensity(Projectile.Center, 5f * Projectile.oldVelocity.Y / 18, 300f);

					int ShockwaveAmount = (int)Projectile.oldVelocity.Y / 5;
