...
{
	public class DavyJonesFridgeProj : ModProjectile
	{
			...
			if (player.channel) 
            {
                ...
                if (Projectile.ai[2] >= ItemGlobal.ActiveItem(player).useTime && player.CheckMana(ItemGlobal.ActiveItem(player), ItemGlobal.ActiveItem(player).mana, false, false))
                {
					...
					if (Projectile.owner == Main.myPlayer)
                    {
                        ...
                        ShootSpeed.X + Main.rand.Next(-3, 4), ShootSpeed.Y + Main.rand.Next(-3, 4), ModContent.ProjectileType<FridgeFish>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 
                        ai1: Main.rand.NextBool(4) ? Main.rand.Next(1, 4) : 0);
                    }
