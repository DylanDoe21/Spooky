...
{ 
    public class TonguebladeSlash : SwordSlashBase
    {
        ...
        {
            Vector2 vector2 = (Projectile.rotation - (float)Math.PI / 4f).ToRotationVector2() * 65f * Projectile.scale;
            Vector2 vector3 = (Projectile.rotation + (float)Math.PI / 4f).ToRotationVector2() * 65f * Projectile.scale;
            float num2 = 65f * Projectile.scale;
