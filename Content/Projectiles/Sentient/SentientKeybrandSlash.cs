...
{ 
    public class SentientKeybrandSlash : SwordSlashBase
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
			...
			Color SlashColor = Color.Lerp(Color.DarkBlue, Color.Crimson, Intensity);

            //these are the slash textures themselves
            Main.spriteBatch.Draw(Texture.Value, vector, (Rectangle?)rectangle, SlashColor * Intensity, Projectile.rotation + Projectile.ai[0] * ((float)Math.PI / 4f) * -1f * (1f - CurrentAI), origin, Scale, effects, 0f);
			Main.spriteBatch.Draw(Texture.Value, vector, (Rectangle?)rectangle, SlashColor * Intensity * 0.3f, Projectile.rotation + Projectile.ai[0] * 0.01f, origin, Scale, effects, 0f);
			Main.spriteBatch.Draw(Texture.Value, vector, (Rectangle?)rectangle, SlashColor * Intensity * 0.3f, Projectile.rotation, origin, Scale, effects, 0f);
			Main.spriteBatch.Draw(Texture.Value, vector, (Rectangle?)rectangle, SlashColor * Intensity * 0.5f, Projectile.rotation, origin, Scale * 0.975f, effects, 0f);
			
            //draw star sparkle and trail on top of the slashes
            for (float Repeats = 0f; Repeats < 10f; Repeats += 1f)
            {
				...
				DrawPrettyStarSparkle(Projectile.Opacity, (SpriteEffects)0, drawpos, Color.White, Color.White, CurrentAI, 0f, 0.5f, 0.5f, 1f, Fade, new Vector2(0f, Utils.Remap(CurrentAI, 0f, 1f, 3f, 0f)) * Scale * 0.75f, Vector2.One * Scale * 0.8f);
			    ...
			    //DrawPrettyStarSparkle(Projectile.Opacity, (SpriteEffects)0, drawpos2, Color.White, Color.White, CurrentAI, 0f, 0.5f, 0.5f, 1f, 0f, new Vector2(2f, Utils.Remap(CurrentAI, 0f, 1f, 4f, 1f)) * Scale * 0.75f, Vector2.One * Scale * 0.75f);
            }
