...
{
    public class ZombieCultistPet : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			if (player.dead)
            {
				player.GetModPlayer<SpookyPlayer>().ZombieCultistPet = false;
            }

			if (player.GetModPlayer<SpookyPlayer>().ZombieCultistPet)
            {
				Projectile.timeLeft = 2;
            }
            ...
            else if (playerFlying)
            {
                ...
                //make booger dusts out of its nose
                Vector2 rotationOrigin = new Vector2(0f, 0f);
                float overrideRotation = rotationOrigin.ToRotation();
                Vector2 dustVelo = new Vector2(0, 0).RotatedBy(overrideRotation);
                Vector2 fromBody = Projectile.Center + new Vector2(-3, -5).RotatedBy(Projectile.rotation);
                int index = Dust.NewDust(fromBody + dustVelo * Projectile.scale, 0, 0, DustID.GreenBlood, 0, 0, 0, Color.White, 1.5f);
