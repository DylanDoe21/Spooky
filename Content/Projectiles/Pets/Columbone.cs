...
{
    public class Columbone : ModProjectile
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            .WithOffset(-9f, 0f).WithSpriteDirection(-1).WhenNotSelected(0, 0);
        }
        ...
        public override void AI()
        {
			...
			if (player.dead)
            {
				player.GetModPlayer<SpookyPlayer>().ColumbonePet = false;
            }

			if (player.GetModPlayer<SpookyPlayer>().ColumbonePet)
            {
				Projectile.timeLeft = 2;
            }
            ...
            if (!playerFlying)
            {
                ...
                float StopDistance = (player.GetModPlayer<SpookyPlayer>().ColumboPet || player.GetModPlayer<SpookyPlayer>().ColumborangePet) ? 70f : 80f;
		...
		{
            Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<ColumboneFinger>(), 0, 0f, Main.myPlayer);
        }
    }
}