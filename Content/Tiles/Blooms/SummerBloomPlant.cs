...
namespace Spooky.Content.Tiles.Blooms
{
	public class SummerBloomPlant : ModTile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(64, 119, 64), name);
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			...
			if (frameX == 216)
			{
				...
				if (frameY == 0) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<SummerOrange>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 54) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<SummerPineapple>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 108) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<SummerSunflower>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 162) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<SummerLemon>(), ShouldDropExtra ? 2 : 1);
			}
		}
