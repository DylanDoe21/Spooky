...
{
	public class TomatoBloomPlant : ModTile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(201, 51, 33), name);
			RegisterItemDrop(ModContent.ItemType<TomatoSeed>());
		...
		{
			PlantTexture ??= ModContent.Request<Texture2D>("Spooky/Content/Tiles/Blooms/TomatoBloomPlant");
			...
			if (tile.TileFrameY == 36 || tile.TileFrameY == 90 || tile.TileFrameY == 144 || tile.TileFrameY == 198)
			{
				...
				if (tile.TileFrameX == 18 || tile.TileFrameX == 72 || tile.TileFrameX == 126 || tile.TileFrameX == 180 || tile.TileFrameX == 234)
				{
					...
					Vector2 offset = new Vector2((PlantTexture.Width() / 2) - 1, (PlantTexture.Height() / 5) - 12);
					...
					DrawPlant(i, j, PlantTexture.Value, new Rectangle(0, 88 * frame, 54, 86), TileGlobal.TileOffset, offset);
				}
			}
		}
