...
{
	public class CemeteryBloomPlant : ModTile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(72, 69, 75), name);
			RegisterItemDrop(ModContent.ItemType<CemeterySeed>());
			DustType = DustID.Ash;
		...
		{
			PlantTexture ??= ModContent.Request<Texture2D>("Spooky/Content/Tiles/Blooms/CemeteryBloomPlant");
			...
			if (tile.TileFrameY == 36 || tile.TileFrameY == 90 || tile.TileFrameY == 144 || tile.TileFrameY == 198)
			{
				...
				if (tile.TileFrameX == 18 || tile.TileFrameX == 72 || tile.TileFrameX == 126 || tile.TileFrameX == 180 || tile.TileFrameX == 234)
				{
					...
					DrawPlant(i, j, PlantTexture.Value, new Rectangle(0, 86 * frame, 56, 84), TileGlobal.TileOffset, offset);
				}
			}
		}
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			...
			if (frameX == 216)
			{
				...
				if (ShouldDropExtra)
				{
					Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<CemeterySeed>());
				}

				if (frameY == 0) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<CemeteryLily>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 54) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<CemeteryMarigold>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 108) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<CemeteryPoppy>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 162) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<CemeteryRose>(), ShouldDropExtra ? 2 : 1);
			}
		}
