...
{
	public class SeaBloomPlant : ModTile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(195, 105, 80), name);
			RegisterItemDrop(ModContent.ItemType<SeaSeed>());
		...
		{
			PlantTexture ??= ModContent.Request<Texture2D>("Spooky/Content/Tiles/Blooms/SeaBloomPlant");
			...
			if (tile.TileFrameY == 36 || tile.TileFrameY == 90 || tile.TileFrameY == 144 || tile.TileFrameY == 198)
			{
				...
				if (tile.TileFrameX == 18 || tile.TileFrameX == 72 || tile.TileFrameX == 126 || tile.TileFrameX == 180 || tile.TileFrameX == 234)
				{
					...
					DrawPlant(i, j, PlantTexture.Value, new Rectangle(0, 80 * frame, 56, 78), TileGlobal.TileOffset, offset);
				}
			}
		}
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			...
			if (frameX == 216)
			{
				...
				if (ShouldDropExtra)
				{
					Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<SeaSeed>());
				}

				if (frameY == 0) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<SeaBarnacle>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 54) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<SeaCucumber>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 108) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<SeaSponge>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 162) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<SeaUrchin>(), ShouldDropExtra ? 2 : 1);
			}
		}
			...
			if (tile.TileFrameX < 216 && Main.rand.NextBool(ShouldGrowFaster ? 12 : 20))
			{
				...
				for (int x = left; x < left + 3; x++)
				{
					for (int y = top; y < top + 3; y++)
					{
						...
						if (CheckTile.LiquidType == LiquidID.Water && CheckTile.LiquidAmount > 0)
						{
							HasWater = true;
						}
						else
