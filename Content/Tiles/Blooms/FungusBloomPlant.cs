...
{
	public class FungusBloomPlant : ModTile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(153, 174, 163), name);
			RegisterItemDrop(ModContent.ItemType<FungusSeed>());
		...
		{
			PlantTexture ??= ModContent.Request<Texture2D>("Spooky/Content/Tiles/Blooms/FungusBloomPlant");
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			...
			if (frameX == 216)
			{
				...
				if (ShouldDropExtra)
				{
					Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<FungusSeed>());
				}

				if (frameY == 0) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<FungusBluePinkgill>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 54) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<FungusDevilTooth>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 108) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<FungusInkCap>(), ShouldDropExtra ? 2 : 1);
				if (frameY == 162) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<FungusRedCage>(), ShouldDropExtra ? 2 : 1);
			}
		}
