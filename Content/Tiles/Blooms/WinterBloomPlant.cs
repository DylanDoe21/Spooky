...
namespace Spooky.Content.Tiles.Blooms
{
	public class WinterBloomPlant : ModTile
	{
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			...
			if (frameX == 216)
			{
				if (frameY == 0) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<WinterBlackberry>());
				if (frameY == 54) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<WinterBlueberry>());
				if (frameY == 108) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<WinterGooseberry>());
				if (frameY == 162) Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i + 1, j) * 16, ModContent.ItemType<WinterStrawberry>());
			}
		}
