...
{
    public class GourdMedium : ModTile
    {
        ...
        public override bool RightClick(int i, int j)
        {
            if (Main.LocalPlayer.HasItem(ModContent.ItemType<PumpkinCarvingKit>()))
            {
                ...
                for (int x = left; x < left + 3; x++)
                {
                    ...
                    {
                        Main.tile[x, y].TileType = (ushort)ModContent.TileType<GourdMediumCarved>();
                    }
                }
            }
    ...
    public class GourdMediumCarved : GourdMedium
    {
        public override void SetStaticDefaults()
        {
            Main.tileSolid[Type] = false;
        ...
        public override bool RightClick(int i, int j)
        {
            if (Main.LocalPlayer.ConsumeItem(ModContent.ItemType<CandleItem>()))
            {
                ...
                for (int x = left; x < left + 3; x++)
                {
                    for (int y = top; y < top + 2; y++)
                    {
                        Main.tile[x, y].TileType = (ushort)ModContent.TileType<GourdMediumCarvedLit>();
                    }
                }
            }
    ...
    public class GourdMediumCarvedLit : GourdMediumCarved
    {
        public override string Texture => "Spooky/Content/Tiles/SpookyBiome/Ambient/GourdMediumCarved";
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch) 
		{
            ...
            Texture2D flameTexture = ModContent.Request<Texture2D>("Spooky/Content/Tiles/SpookyBiome/Ambient/GourdMediumCarvedGlow").Value;
