...
namespace Spooky.Content.Tiles.SpookyBiome.Gourds
{
    public class GourdRed : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            AddMapEntry(new Color(184, 47, 26));
            DustType = 288;
        ...
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 32, 16, ModContent.ItemType<GourdBlockRedItem>(), Main.rand.Next(15, 26));
        }
            ...
            if (ItemGlobal.ActiveItem(player).type == ModContent.ItemType<PumpkinCarvingKit>())
            {
                ...
                for (int x = left; x < left + 3; x++)
                {
                    for (int y = top; y < top + 3; y++)
                    {
                        ...
                        tile.TileType = (ushort)ModContent.TileType<GourdRedCarved>();
                    }
                }
    ...
    public class GourdRedCarved : GourdRed
    {
        public override void SetStaticDefaults()
        {
            Main.tileSolid[Type] = false;
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            if (ItemGlobal.ActiveItem(player).type == ModContent.ItemType<CandleItem>() && player.ConsumeItem(ModContent.ItemType<CandleItem>()))
            {
                ...
                for (int x = left; x < left + 3; x++)
                {
                    ...
                    {
                        Tile tile = Framing.GetTileSafely(x, y);
                        tile.TileType = (ushort)ModContent.TileType<GourdRedCarvedLit>();
                    }
                }
    ...
    public class GourdRedCarvedLit : GourdRedCarved
    {
        public override string Texture => "Spooky/Content/Tiles/SpookyBiome/Gourds/GourdRedCarved";
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch) 
		{
            ...
            GlowTexture ??= ModContent.Request<Texture2D>("Spooky/Content/Tiles/SpookyBiome/Gourds/GourdRedCarvedGlow");
