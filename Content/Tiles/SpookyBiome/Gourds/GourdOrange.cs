...
{
    [LegacyName("GourdMedium")]
    public class GourdOrange : ModTile
    {
        ...
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 32, 16, ModContent.ItemType<GourdBlockOrangeItem>(), Main.rand.Next(15, 26));
        }
            ...
            if (ItemGlobal.ActiveItem(player).type == ModContent.ItemType<PumpkinCarvingKit>())
            {
                ...
                for (int x = left; x < left + 3; x++)
                {
                    for (int y = top; y < top + 2; y++)
                    {
                        ...
                        tile.TileType = (ushort)ModContent.TileType<GourdOrangeCarved>();
                    }
                }
    ...
    [LegacyName("GourdMediumCarved")]
    public class GourdOrangeCarved : GourdOrange
    {
        public override void SetStaticDefaults()
        {
            Main.tileSolid[Type] = false;
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            if (ItemGlobal.ActiveItem(player).type == ModContent.ItemType<CandleItem>() && player.ConsumeItem(ModContent.ItemType<CandleItem>()))
            {
                ...
                for (int x = left; x < left + 3; x++)
                {
                    ...
                    {
                        Tile tile = Framing.GetTileSafely(x, y);
                        tile.TileType = (ushort)ModContent.TileType<GourdOrangeCarvedLit>();
                    }
                }
    ...
    [LegacyName("GourdMediumCarvedLit")]
    public class GourdOrangeCarvedLit : GourdOrangeCarved
    {
        public override string Texture => "Spooky/Content/Tiles/SpookyBiome/Gourds/GourdOrangeCarved";
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch) 
		{
            ...
            GlowTexture ??= ModContent.Request<Texture2D>("Spooky/Content/Tiles/SpookyBiome/Gourds/GourdOrangeCarvedGlow");
