...
{
    public class GourdLime : ModTile
    {
        ...
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 32, 16, ModContent.ItemType<GourdBlockLimeItem>(), Main.rand.Next(15, 26));
        }
            ...
            if (ItemGlobal.ActiveItem(player).type == ModContent.ItemType<PumpkinCarvingKit>())
            {
                ...
                for (int x = left; x < left + 3; x++)
                {
                    for (int y = top; y < top + 3; y++)
                    {
                        ...
                        tile.TileType = (ushort)ModContent.TileType<GourdLimeCarved>();
                    }
                }
    ...
    public class GourdLimeCarved : GourdLime
    {
        public override void SetStaticDefaults()
        {
            Main.tileSolid[Type] = false;
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            if (ItemGlobal.ActiveItem(player).type == ModContent.ItemType<CandleItem>() && player.ConsumeItem(ModContent.ItemType<CandleItem>()))
            {
                ...
                for (int x = left; x < left + 3; x++)
                {
                    ...
                    {
                        Tile tile = Framing.GetTileSafely(x, y);
                        tile.TileType = (ushort)ModContent.TileType<GourdLimeCarvedLit>();
                    }
                }
    ...
    public class GourdLimeCarvedLit : GourdLimeCarved
    {
        public override string Texture => "Spooky/Content/Tiles/SpookyBiome/Gourds/GourdLimeCarved";
		...
		{
            GlowTexture ??= ModContent.Request<Texture2D>("Spooky/Content/Tiles/SpookyBiome/Gourds/GourdLimeCarvedGlow");
