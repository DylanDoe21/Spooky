...
{
    [LegacyName("GourdGiant")]
    public class GourdLimeOrange : ModTile
    {
        ...
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 32, 16, ModContent.ItemType<GourdBlockLimeOrangeItem>(), Main.rand.Next(15, 26));
        }
            ...
            if (ItemGlobal.ActiveItem(player).type == ModContent.ItemType<PumpkinCarvingKit>())
            {
                ...
                for (int x = left; x < left + 3; x++)
                {
                    for (int y = top; y < top + 4; y++)
                    {
                        ...
                        tile.TileType = (ushort)ModContent.TileType<GourdLimeOrangeCarved>();
                    }
                }
    ...
    [LegacyName("GourdGiantCarved")]
    public class GourdLimeOrangeCarved : GourdLimeOrange
    {
        public override void SetStaticDefaults()
        {
            Main.tileSolid[Type] = false;
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            if (ItemGlobal.ActiveItem(player).type == ModContent.ItemType<CandleItem>() && player.ConsumeItem(ModContent.ItemType<CandleItem>()))
            {
                ...
                for (int x = left; x < left + 3; x++)
                {
                    ...
                    {
                        Tile tile = Framing.GetTileSafely(x, y);
                        tile.TileType = (ushort)ModContent.TileType<GourdLimeOrangeCarvedLit>();
                    }
                }
    ...
    [LegacyName("GourdGiantCarvedLit")]
    public class GourdLimeOrangeCarvedLit : GourdLimeOrangeCarved
    {
        public override string Texture => "Spooky/Content/Tiles/SpookyBiome/Gourds/GourdLimeOrangeCarved";
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch) 
		{
            ...
            GlowTexture ??= ModContent.Request<Texture2D>("Spooky/Content/Tiles/SpookyBiome/Gourds/GourdLimeOrangeCarvedGlow");
