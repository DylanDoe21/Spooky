...
{
	public class DampMushroomVines : ModTile
	{
        ...
        {
			int[] ValidTiles = { ModContent.TileType<DampMushroomVines>(), ModContent.TileType<DampMushroomGrass>() };
		...
		public override void RandomUpdate(int i, int j)
		{
			...
			if (WorldGen.genRand.NextBool(12) && !tileBelow.HasTile && tileBelow.LiquidType != LiquidID.Lava)
            {
				...
				while (Test > j - 10) 
                {
					...
					else if (!testTile.HasTile || testTile.TileType != ModContent.TileType<DampMushroomGrass>()) 
                    {
						Test--;
						continue;
					}
