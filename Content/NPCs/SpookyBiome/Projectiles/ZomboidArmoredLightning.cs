...
{
    public class ZomboidArmoredLightning : ModProjectile
    {
		...
		{
			if (target.type == ModContent.NPCType<ZomboidArmored>())
			{
				for (int numDusts = 0; numDusts < 20; numDusts++)
				{
					int Electricity = Dust.NewDust(target.position, target.width, target.height, DustID.Electric, 0f, 0f, 100, default, 1f);
					Main.dust[Electricity].velocity *= 2f;
