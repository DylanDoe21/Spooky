...
{
    public class OrchidPurpleBig : ModNPC  
    {
        int MoveSpeedX;
        int MoveSpeedY;
        ...
        public override void SetStaticDefaults()
        {
            ...
            {
                CustomTexturePath = "Spooky/Content/NPCs/NPCDisplayTextures/OrchidPurpleBigBestiary",
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) 
        {
            ...
            {
				new FlavorTextBestiaryInfoElement("Mods.Spooky.Bestiary.OrchidPurpleBig"),
        ...
        public void ChasePlayer(Player player, NPC Parent, int MaxSpeed, float Acceleration)
        {
            ...
            else
            {
                ...
                if (NPC.Center.Y >= Parent.Center.Y - 180 && MoveSpeedY >= -MaxSpeed)
                {
                    MoveSpeedY--;
                }
                else if (NPC.Center.Y <= Parent.Center.Y - 180 && MoveSpeedY <= MaxSpeed)
                {
                    MoveSpeedY++;
                }
        ...
        public override void HitEffect(NPC.HitInfo hit) 
        {
            if (NPC.life <= 0) 
            {
                for (int numGores = 1; numGores <= 5; numGores++)
                {
                    ...
                    {
                        Gore.NewGore(NPC.GetSource_Death(), NPC.Center, NPC.velocity, ModContent.Find<ModGore>("Spooky/OrchidPurpleBigGore" + numGores).Type);
                    }
                }
