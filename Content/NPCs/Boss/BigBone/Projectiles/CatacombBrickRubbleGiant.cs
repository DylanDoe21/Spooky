...
{
    public class CatacombBrickRubbleGiant : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            {
                TrailTexture ??= ModContent.Request<Texture2D>("Spooky/Content/NPCs/Boss/BigBone/Projectiles/CatacombBrickRubbleGiantTrail");
		...
		public override void OnKill(int timeLeft)
		{
            ...
            SoundEngine.PlaySound(SoundID.NPCDeath43 with { Pitch = 1f }, Projectile.Center);

            for (int numProj = 0; numProj < 3; numProj++)
            {
                int ShootSpeedX = Main.rand.Next(-4, 5);
                int ShootSpeedY = Main.rand.Next(-20, -14);

                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, new Vector2(ShootSpeedX, ShootSpeedY), 
                    ModContent.ProjectileType<CatacombBrickRubble>(), Projectile.damage, Projectile.knockBack, ai0: 60, ai2: 20);
                }
            }
