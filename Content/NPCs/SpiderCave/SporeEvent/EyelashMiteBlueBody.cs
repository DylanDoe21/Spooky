...
{
    public class EyelashMiteBlueBody1 : ModNPC
    {
        ...
        public override bool PreAI()
        {
			...
			if (!Parent.active || Parent.type != ModContent.NPCType<EyelashMiteBlueHead>())
            {
                if (Main.netMode != NetmodeID.Server) 
                {
                    Gore.NewGore(NPC.GetSource_Death(), NPC.Center, NPC.velocity, ModContent.Find<ModGore>("Spooky/EyelashMiteBlueBodyGore1").Type);
                }
    ...
    public class EyelashMiteBlueBody2 : EyelashMiteBlueBody1
    {
        private static Asset<Texture2D> NPCTexture;
        ...
        public override bool PreAI()
        {
			...
			if (!Parent.active || Parent.type != ModContent.NPCType<EyelashMiteBlueHead>())
            {
                if (Main.netMode != NetmodeID.Server) 
                {
                    Gore.NewGore(NPC.GetSource_Death(), NPC.Center, NPC.velocity, ModContent.Find<ModGore>("Spooky/EyelashMiteBlueBodyGore2").Type);
                }
