...
{
    public class EyelashMiteBlueTail : ModNPC
    {
        ...
        public override bool PreAI()
        {
			...
			if (!Parent.active || Parent.type != ModContent.NPCType<EyelashMiteBlueHead>())
            {
                ...
                {
                    Gore.NewGore(NPC.GetSource_Death(), NPC.Center, NPC.velocity, ModContent.Find<ModGore>("Spooky/EyelashMiteBlueTailGore").Type);
                }
			...
			{
				NPC.Center = SegmentParent.Center - SegmentCenter.SafeNormalize(Vector2.Zero) * 22f;
			}
