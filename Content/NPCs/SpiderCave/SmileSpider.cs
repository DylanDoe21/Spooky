...
{
	public class SmileSpider : ModNPC
	{
		List<SmileSpiderLeg> legs;
		...
		public override void SetStaticDefaults()
        {
            ...
            {
                CustomTexturePath = "Spooky/Content/NPCs/NPCDisplayTextures/SmileSpiderBestiary"
            };
        }
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) 
        {
            ...
            {
				new FlavorTextBestiaryInfoElement("Mods.Spooky.Bestiary.SmileSpider"),
		...
		public void UpdateSpiderLegs()
		{
			if (legs == null)
			{
				legs = new List<SmileSpiderLeg>();
				Vector2[] Origin = { new Vector2(15, -12), new Vector2(15, -12) }; //this is for the origin point of the leg relative to the npc
				Vector2[] Destination = { new Vector2(110, -64), new Vector2(62, -64) }; //this is the destination where the very bottom point of the leg should go when it moves

				for (int numLegs = -1; numLegs <= 1; numLegs += 2)
				{
					for (int i = 0; i < Destination.Length; i++)
					{
						legs.Add(new SmileSpiderLeg(new Vector2(Destination[i].X, Destination[i].Y * numLegs), new Vector2(Origin[i].X, Origin[i].Y * numLegs), numLegs));
					}
				}
			}
			else
		...
		public override void HitEffect(NPC.HitInfo hit) 
        {
			...
			if (NPC.life <= 0)
            {
                ...
                for (int numGores = 1; numGores <= 3; numGores++)
                {
                    ...
                    {
                        Gore.NewGore(NPC.GetSource_Death(), NPC.Center, NPC.velocity, ModContent.Find<ModGore>("Spooky/SmileSpiderGore" + numGores).Type);
                    }
                }
			}
		}
	...
	public class SmileSpiderLeg
	{
		...
		public SmileSpiderLeg(Vector2 Startleg, Vector2 BodyPosition, int LegSide)
		{
		...
		{
			int LegSegmentLength = 24;
			int LegClawLength = 34;
			...
			if (!SpawnGore)
			{
				LegSegmentTexture ??= ModContent.Request<Texture2D>("Spooky/Content/NPCs/SpiderCave/SmileSpiderLegSegment");
				LegClawTexture ??= ModContent.Request<Texture2D>("Spooky/Content/NPCs/SpiderCave/SmileSpiderLegFront");
			...
			else
			{
				if (Main.netMode != NetmodeID.Server)
				{
					Gore.NewGore(NPC.GetSource_Death(), start, new Vector2(Main.rand.Next(-2, 3), Main.rand.Next(-2, 3)), ModContent.Find<ModGore>("Spooky/SmileSpiderLegGore1").Type);
					Gore.NewGore(NPC.GetSource_Death(), halfway2, new Vector2(Main.rand.Next(-2, 3), Main.rand.Next(-2, 3)), ModContent.Find<ModGore>("Spooky/SmileSpiderLegGore2").Type);
				}
			}
		}
	}
