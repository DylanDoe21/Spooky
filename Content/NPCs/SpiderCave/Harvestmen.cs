...
{
	public class Harvestmen : ModNPC
	{
		List<HarvestmenLeg> legs;
		...
		public override void SetStaticDefaults()
        {
            ...
            {
                CustomTexturePath = "Spooky/Content/NPCs/NPCDisplayTextures/HarvestmenBestiary",
		...
		public void UpdateSpiderLegs()
		{
			if (legs == null)
			{
				legs = new List<HarvestmenLeg>();
				Vector2[] Origin = { new Vector2(4, -12), new Vector2(2, -12), new Vector2(0, -12) }; //this is for the origin point of the leg relative to the npc
				Vector2[] Destination = { new Vector2(112, -65), new Vector2(70, -70), new Vector2(45, -48) }; //this is the destination where the very bottom point of the leg should go when it moves

				for (int numLegs = -1; numLegs <= 1; numLegs += 2)
				{
					for (int i = 0; i < Destination.Length; i++)
					{
						legs.Add(new HarvestmenLeg(new Vector2(Destination[i].X, Destination[i].Y * numLegs), new Vector2(Origin[i].X, Origin[i].Y * numLegs), numLegs));
					}
				}
			}
			else
		...
		public override void AI()
		{
			...
			if (NPC.ai[1] == 0)
			{
				...
				if (NPC.ai[0] == 0)
				{
					...
					{
						Vector2 desiredVelocity = NPC.DirectionTo(SavePosition) * 2;
	...
	public class HarvestmenLeg
	{
		...
		public HarvestmenLeg(Vector2 Startleg, Vector2 BodyPosition, int LegSide)
		{
		...
		{
			int LegSegmentLength = 46;
			int LegClawLength = 54;
			...
			if (!SpawnGore)
			{
				LegSegmentTexture ??= ModContent.Request<Texture2D>("Spooky/Content/NPCs/SpiderCave/HarvestmenLegSegment");
				LegClawTexture ??= ModContent.Request<Texture2D>("Spooky/Content/NPCs/SpiderCave/HarvestmenLegFront");
			...
			else
			{
				if (Main.netMode != NetmodeID.Server)
				{
					Gore.NewGore(NPC.GetSource_Death(), start, new Vector2(Main.rand.Next(-2, 3), Main.rand.Next(-2, 3)), ModContent.Find<ModGore>("Spooky/HarvestmenLegGore1").Type);
					Gore.NewGore(NPC.GetSource_Death(), start, new Vector2(Main.rand.Next(-2, 3), Main.rand.Next(-2, 3)), ModContent.Find<ModGore>("Spooky/HarvestmenLegGore2").Type);
					Gore.NewGore(NPC.GetSource_Death(), halfway2, new Vector2(Main.rand.Next(-2, 3), Main.rand.Next(-2, 3)), ModContent.Find<ModGore>("Spooky/HarvestmenLegGore3").Type);
					Gore.NewGore(NPC.GetSource_Death(), halfway2, new Vector2(Main.rand.Next(-2, 3), Main.rand.Next(-2, 3)), ModContent.Find<ModGore>("Spooky/HarvestmenLegGore4").Type);
				}
			}
		}
	}
