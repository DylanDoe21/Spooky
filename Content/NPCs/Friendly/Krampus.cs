		...
		public override void AI()
		{
			...
			*/
			...
			foreach (var player in Main.player)
            {
                ...
                if (player.talkNPC == NPC.whoAmI && NPC.Hitbox.Intersects(new Rectangle((int)Main.MouseWorld.X - 1, (int)Main.MouseWorld.Y - 1, 1, 1)) &&
				NPC.Distance(player.Center) <= 150f && !Main.mapFullscreen && !Main.dedServ) //&& Main.myPlayer == player.whoAmI)
                {
