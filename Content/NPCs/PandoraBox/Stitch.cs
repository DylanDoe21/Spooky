...
namespace Spooky.Content.NPCs.PandoraBox
{
    public class Stitch : ModNPC
    {
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) 
        {
            ...
            {
				new FlavorTextBestiaryInfoElement("Mods.Spooky.Bestiary.Stitch"),
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            for (int i = 0; i < 360; i += 90)
            {
                ...
                for (int repeats = 0; repeats < 4; repeats++)
                {
                    ...
                    spriteBatch.Draw(NPCTexture.Value, DrawPosition, NPC.frame, color * fade, NPC.rotation, NPC.frame.Size() / 2, NPC.scale * 1.2f, effects, 0f);
                }
            }

            Color color2 = new Color(125 - NPC.alpha, 125 - NPC.alpha, 125 - NPC.alpha, 0).MultiplyRGBA(Color.Cyan);

            Main.spriteBatch.Draw(NPCTexture.Value, NPC.Center - screenPos + new Vector2(0, NPC.gfxOffY + 4), NPC.frame, color2, NPC.rotation, NPC.frame.Size() / 2f, NPC.scale * 1.2f, effects, 0f);
        ...
        public override void AI()
		{
            ...
            if (!SpawnedWand)
            {
                NPC.ai[1] = NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<StitchWand>(), ai1: NPC.whoAmI);
            ...
            switch ((int)NPC.ai[0])
            {
                ...
                //cast at the player, ai is handled in stitch's wand
                case 1:
                {
                    NPC.localAI[0]++;
